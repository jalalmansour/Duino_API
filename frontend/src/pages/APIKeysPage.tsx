  ...
  const fetchKeys = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${apiUrl}/v1/keys`, {
        headers: { 'X-API-Key': authKey, 'ngrok-skip-browser-warning': 'true' },
      })
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        ...
        <Card className="bg-ink border-slate rounded-[6px] shadow-none h-full flex flex-col">
          ...
          <CardContent className="p-0 flex-1 relative flex flex-col h-full bg-void">
            ...
            <div className="flex-1 overflow-auto selection:bg-violet/30">
              <SyntaxHighlighter
                language={activeLang === 'rest' ? 'http' : activeLang === 'curl' ? 'bash' : activeLang === 'dotnet' ? 'csharp' : activeLang}
                style={vscDarkPlus}
                customStyle={{ margin: 0, padding: '2rem', background: 'transparent', fontSize: '13px' }}
                codeTagProps={{ className: "font-mono" }}
              >
                {getCodeSnippet(activeLang, apiUrl, initialApiKey)}
              </SyntaxHighlighter>
            </div>
          </CardContent>
        </Card>
